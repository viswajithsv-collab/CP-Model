...
class PulmonaryModel:
    ...
    def compute_derivatives(self, t, Ppa, Ppvb, Ppv, Qin, Pla, P_thor=0.0, dP_thor=0.0):
        """
        Compute all pulmonary derivatives for Euler integration

        State variables (passed in):
        - Ppa: pulmonary arterial pressure (mmHg)
        - Ppvb: pulmonary vascular bed pressure (mmHg)
        - Ppv: pulmonary venous pressure (mmHg)

        Coupling inputs:
        - Qin: inflow from right ventricle (mL/s)
        - Pla: left atrial pressure (mmHg)
        - P_thor: intrathoracic pressure (mmHg)
        - dP_thor: intrathoracic pressure derivative (mmHg/s)

        Returns:
        - derivatives: dict with dPpa, dPpvb, dPpv
        - outputs: dict with flows and volumes
        """

        # 1. Pulmonary artery (A18-A20)
        Qpa = (Ppa - Ppvb) / self.Rpa
        dPpa = (Qin - Qpa) / self.Cpa + dP_thor
        Vpa = self.Cpa * (Ppa - P_thor) + self.Vu_pa

        # 2. Pulmonary vascular bed (A21-A26)
        Qpp = (Ppvb - Ppv) / self.Rpp
        Qps = (Ppvb - Ppv) / self.Rps
        Ctotal = self.Cpp + self.Cps
        dPpvb = (Qpa - Qpp - Qps) / Ctotal + dP_thor
        Vpp = self.Cpp * (Ppvb - P_thor) + self.Vu_pp
        Vps = self.Cps * (Ppvb - P_thor) + self.Vu_ps

        # 3. Pulmonary veins (A27-A29)
        Qpv = (Ppv - Pla) / self.Rpv
        dPpv = (Qpp + Qps - Qpv) / self.Cpv + dP_thor
        Vpv = self.Cpv * (Ppv - P_thor) + self.Vu_pv
