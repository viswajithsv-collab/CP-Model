...
class ControlSystemModel:
    ...
    def __init__(self, params):
        ...
        # =============================================================================
        # O2 AUTOREGULATION FOR RESISTANCE (Eq 16-18, Magosso 2002)
        # =============================================================================
        self.Ramp_n = params.get('Ramp_n', 4.48)  # Baseline active muscle peripheral resistance (mmHg·s/mL)
        self.Rhp_n = params.get('Rhp_n', 19.71)  # Baseline coronary peripheral resistance (mmHg·s/mL)
        self.g_am_O2 = params.get('g_am_O2', 0.8)  # Active muscle O2 autoregulation gain
        self.g_h_O2 = params.get('g_h_O2', 0.8)  # Coronary O2 autoregulation gain
        self.tau_am_O2 = params.get('tau_am_O2', 20.0)  # Active muscle O2 time constant (s)
        self.tau_h_O2 = params.get('tau_h_O2', 10.0)  # Coronary O2 time constant (s)
        self.Cv_O2_n = params.get('Cv_O2_n', 0.14)  # Normal venous O2 concentration (mL O2/mL blood)

        # Metabolic state for muscle resistance (Eq 20-21)
        self.tau_M = params.get('tau_M', 60.0)  # Metabolism time constant (s)
        self.g_M = params.get('g_M', 5.0)  # Metabolism gain for exercise

        # =============================================================================
        # VENTILATION CONTROL (Eq 25-33, Magosso 2002)
        # =============================================================================
        # Metabolic ventilation sigmoid (Eq 25-26)
        self.tau_phi_met = params.get('tau_phi_met', 40.0)  # Metabolic ventilation time constant (s)
        self.D_phi = params.get('D_phi', 15.0)  # Metabolic ventilation delay (s)
        self.phi_met_min = params.get('phi_met_min', 0.0)  # Min metabolic stimulus
        self.phi_met_max = params.get('phi_met_max', 10.0)  # Max metabolic stimulus
        self.I_0_met = params.get('I_0_met', 0.35)  # Sigmoid inflection point
        self.k_phi_met = params.get('k_phi_met', 0.08)  # Sigmoid slope parameter

        # Ventilation fast/slow dynamics (Eq 27-31)
        self.A_Vdot = params.get('A_Vdot', 30.0)  # Linear coefficient (L/min)
        self.B_Vdot = params.get('B_Vdot', 50.0)  # Quadratic coefficient (L/min)
        self.tau_Vdot_slow = params.get('tau_Vdot_slow', 65.0)  # Slow component time constant (s)
        self.fast_frac = params.get('fast_frac', 0.45)  # Fast fraction of steady-state

        # Tidal volume (Eq 32-33)
        self.VT_n = params.get('VT_n', 0.5)  # Baseline tidal volume (L)
        self.g_VT = params.get('g_VT', 0.5)  # Tidal volume gain
    ...
    def autoregulationO2StaticActive(self, Cv_O2, Cv_O2_n, g_O2):
        """
        Static O2 autoregulation for active muscle (Magosso 2002, Eq 18).

        Uses venous O2 concentration instead of tissue pO2.
        Vasodilation when venous O2 drops (increased extraction).

        Args:
            Cv_O2: Venous O2 concentration (mL O2/mL blood)
            Cv_O2_n: Normal venous O2 concentration (mL O2/mL blood)
            g_O2: Gain coefficient

        Returns:
            float: Static O2 autoregulation signal
        """
        return -g_O2 * (Cv_O2 - Cv_O2_n)

    def metabolicAutoregulationStatic(self, I, g_M):
        """
        Static metabolic autoregulation for active muscle (Magosso 2002, Eq 20-21).

        Exercise intensity drives metabolic vasodilation.

        Args:
            I: Exercise intensity (0-1)
            g_M: Metabolic gain coefficient

        Returns:
            float: Static metabolic autoregulation signal
        """
        return g_M * I
    ...
    def restingMuscleAutoregulation(self, pO2_rm, pCO2_rm, x_rm_O2, x_rm_CO2):
        """
        Resting muscle autoregulation response (O2 + CO2).

        For non-exercising muscle tissue - standard O2/CO2 autoregulation.

        Args:
            pO2_rm: Resting muscle tissue O2 (mmHg)
            pCO2_rm: Resting muscle tissue CO2 (mmHg)
            x_rm_O2: Resting muscle O2 autoregulation state
            x_rm_CO2: Resting muscle CO2 autoregulation state

        Returns:
            tuple: (sigma_rm, dx_rm_O2_dt, dx_rm_CO2_dt)
        """
        sigma_O2_static = self.autoregulationO2Static(pO2_rm, self.pO2_n, self.G_mus_O2)
        sigma_CO2_static = self.autoregulationCO2Static(pCO2_rm, self.pCO2_n, self.G_mus_CO2)

        dx_rm_O2_dt = self.autoregulationDynamics(sigma_O2_static, x_rm_O2, self.tau_mus_O2)
        dx_rm_CO2_dt = self.autoregulationDynamics(sigma_CO2_static, x_rm_CO2, self.tau_mus_CO2)

        sigma_rm = x_rm_O2 + x_rm_CO2

        return sigma_rm, dx_rm_O2_dt, dx_rm_CO2_dt

    def activeMuscleAutoregulation(self, Cv_am_O2, pCO2_am, x_am_O2, x_am_CO2, x_met, I):
        """
        Active muscle autoregulation with O2, CO2, and metabolic components.

        Extends Magosso 2002 Eq 16 to include CO2 for physiological completeness.

        Args:
            Cv_am_O2: Active muscle venous O2 concentration (mL O2/mL blood)
            pCO2_am: Active muscle tissue CO2 (mmHg)
            x_am_O2: O2 autoregulation state
            x_am_CO2: CO2 autoregulation state
            x_met: Metabolic autoregulation state
            I: Exercise intensity (0-1)

        Returns:
            tuple: (sigma_am, dx_am_O2_dt, dx_am_CO2_dt, dx_met_dt)
        """
        # O2 autoregulation (Eq 18) - uses venous concentration
        sigma_am_O2_static = self.autoregulationO2StaticActive(Cv_am_O2, self.Cv_O2_n, self.g_am_O2)
        dx_am_O2_dt = self.autoregulationDynamics(sigma_am_O2_static, x_am_O2, self.tau_am_O2)

        # CO2 autoregulation - same as resting muscle
        sigma_am_CO2_static = self.autoregulationCO2Static(pCO2_am, self.pCO2_n, self.G_mus_CO2)
        dx_am_CO2_dt = self.autoregulationDynamics(sigma_am_CO2_static, x_am_CO2, self.tau_mus_CO2)

        # Metabolic autoregulation (Eq 20-21)
        sigma_met_static = self.metabolicAutoregulationStatic(I, self.g_M)
        dx_met_dt = self.autoregulationDynamics(sigma_met_static, x_met, self.tau_M)

        # Combined autoregulation signal (extended Eq 16)
        sigma_am = x_am_O2 + x_am_CO2 + x_met

        return sigma_am, dx_am_O2_dt, dx_am_CO2_dt, dx_met_dt
    ...
    def activeMuscleVenousResistanceEffect(self, P_im, R_mv_baseline=0.1):
        """
    ...
    def activeMuscleVenousVolumeEffect(self, P_im, Vu_muscle_baseline=200.0):
        """
    ...
    def exerciseVentilationResponse(self, I, x_Vdot_slow):
        ...
        # ΔV̇_steady = A*I + B*I² (Eq 31)
        delta_Vdot_steady = self.A_vent * I + self.B_vent * I * I

        # Fast component (immediate, 45% of steady-state) (Eq 29)
        delta_Vdot_fast = self.fast_fraction * delta_Vdot_steady

        # Slow component dynamics (Eq 30)
        ...
        # Total ventilation change (Eq 27-28)
        delta_Vdot = delta_Vdot_fast + x_Vdot_slow
        Vdot = self.Vdot_n + delta_Vdot

        # Tidal volume (Eq 32-33)
        # ΔVT = (g_VT * I / ΔV̇_steady) * ΔV̇
        if delta_Vdot_steady > 0.01 and I > 0.01:
            delta_VT = (self.g_VT * I / delta_Vdot_steady) * delta_Vdot
        else:
            delta_VT = 0.0
        VT = self.VT_n + delta_VT

        return Vdot, VT, dx_Vdot_slow_dt
    ...
    def compute_derivatives(self, t, state, inputs):
        ...
        x_rm_O2 = state.get('x_rm_O2', 0.0)
        x_rm_CO2 = state.get('x_rm_CO2', 0.0)
        x_am_O2 = state.get('x_am_O2', 0.0)
        x_am_CO2 = state.get('x_am_CO2', 0.0)
        x_met = state.get('x_met', 0.0)
        ...
        pO2_rm = inputs.get('pO2_rm', 35.0)
        pCO2_rm = inputs.get('pCO2_rm', 45.0)
        Cv_am_O2 = inputs.get('Cv_am_O2', self.Cv_O2_n)
        pCO2_am = inputs.get('pCO2_am', 45.0)
        I = inputs.get('I', 0.0)
        ...
        sigma_rm, dx_rm_O2_dt, dx_rm_CO2_dt = self.restingMuscleAutoregulation(pO2_rm, pCO2_rm, x_rm_O2, x_rm_CO2)
        sigma_am, dx_am_O2_dt, dx_am_CO2_dt, dx_met_dt = self.activeMuscleAutoregulation(Cv_am_O2, pCO2_am, x_am_O2,
                                                                                         x_am_CO2, x_met, I)
        ...
        derivatives['x_rm_O2'] = dx_rm_O2_dt
        derivatives['x_rm_CO2'] = dx_rm_CO2_dt
        derivatives['x_am_O2'] = dx_am_O2_dt
        derivatives['x_am_CO2'] = dx_am_CO2_dt
        derivatives['x_met'] = dx_met_dt
        
        # =====================================================================
        # 5. EXERCISE VENTILATION
        # =====================================================================

        Vdot, VT_exercise, dx_Vdot_slow_dt = self.exerciseVentilationResponse(I, x_Vdot_slow)
    ...
    def getStateNames(self):
        ...
        return [
            ...
            'x_R', 'x_V', 'x_E',
            'x_Ts', 'x_Tv',
            'x_br_O2', 'x_br_CO2', 'x_cor_O2', 'x_cor_CO2',
            'x_rm_O2', 'x_rm_CO2', 'x_am_O2', 'x_am_CO2', 'x_met',
            'x_Vdot_slow'
        ]
